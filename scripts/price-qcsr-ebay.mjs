...
async function searchWithRetry(args) {
  for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
    try {
      ...
      console.log(
        `  retry ${attempt + 1}/${maxRetries} after ${Math.round(
          waitMs,
        )}ms (${error.status})`,
      );
      await sleep(waitMs);
...
function priceFromItems(card, items) {
  const matches = items
  ...
  return {
    ...
    priceText: `${currency}${avg.toFixed(2)} avg (${sample.length})`,
  };
}
...
async function main() {
  ...
  const cards = db
    .prepare(
      `
        SELECT id, name, rarity, price_text AS priceText
        FROM cards
        WHERE rarity = 'Quarter Century Secret Rare'
        ORDER BY id ASC
      `,
    )
    .all();
  ...
  if (cards.length === 0) {
    console.log("No QCSR cards to reprice.");
    return;
  }
  ...
  const updateSearchUrl = db.prepare(
    "UPDATE cards SET price_text = NULL, ebay_search_url = ?, updated_at = ? WHERE id = ?",
  );
  ...
  console.log(
    `Repricing ${cards.length} QCSR cards with robust outlier filtering and adaptive backoff.`,
  );

  for (const [index, card] of cards.entries()) {
    const prefix = `[${index + 1}/${cards.length}]`;
    const ebaySearchUrl = buildEbaySearchUrl(card);

    try {
      const items = await searchWithRetry({ token, marketplaceId, card });
      const price = priceFromItems(card, items);

      if (price === null) {
        updateSearchUrl.run(ebaySearchUrl, nowUnixSeconds(), card.id);
        skipped += 1;
        console.log(`${prefix} skip ${card.id}: ${card.name} - no matching listings`);
      } else {
        updatePrice.run(price.priceText, ebaySearchUrl, nowUnixSeconds(), card.id);
        priced += 1;
        console.log(`${prefix} price ${card.id}: ${card.name} -> ${price.priceText}`);
      }
    } catch (error) {
      errors += 1;
      console.log(
        `${prefix} error ${card.id}: ${card.name} - ${
  ...
  console.log("");
  console.log(
    `Done in ${Math.floor(seconds / 60)}m ${seconds % 60}s. Priced ${priced}, skipped ${skipped}, errors ${errors}.`,
  );
}
