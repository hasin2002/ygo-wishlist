...
function seededSnapshot(): RecordsSnapshot {
  const targets: WishlistTarget[] = [
    ...
    {
      id: "target-preview-blue-eyes-partial",
      name: "Blue-Eyes Alternative White Dragon",
      rarity: "Gold Rare",
      edition: "1st Edition",
      desiredQuantity: 3,
      imageUrl: "https://images.ygoprodeck.com/images/cards/38517737.jpg",
      tcgplayerUrl: "https://www.tcgplayer.com/",
      marketPricePence: 2499,
    },
  ];
  const printings: CardPrinting[] = [
    ...
    {
      id: "printing-preview-blue-eyes-partial",
      targetId: targets[3].id,
      setName: "Movie Pack Gold Edition",
      setCode: "MVP1-ENG46",
      tcgplayerUrl: targets[3].tcgplayerUrl,
      imageUrl: targets[3].imageUrl,
    },
  ];
  const records: RecordEntry[] = [
    ...
    {
      id: "record-preview-partial-costs",
      type: "purchase",
      status: "active",
      date: "2026-07-18",
      title: "Three-card allocation review",
      source: "Card show",
      listingUrl: null,
      amountPence: 7500,
      amountKnown: true,
      notes: "The Purchase total is known, but one physical Copy still needs an allocation.",
      revision: 1,
      createdAt: "2026-07-18T12:00:00.000Z",
      lines: [{
        id: "line-preview-partial-costs",
        kind: "card",
        name: "Blue-Eyes Alternative White Dragon",
        quantity: 3,
        allocationPence: 7500,
        entityIds: ["copy-preview-partial-1", "copy-preview-partial-2", "copy-preview-partial-3"],
        detail: "MVP1-ENG46 · allocation review",
      }],
    },
  ];
  ...
  const snapshot: RecordsSnapshot = {
    ...
    copies: [
      ...
      {
        id: "copy-preview-partial-1",
        printingId: printings[3].id,
        acquiredRecordId: "record-preview-partial-costs",
        soldRecordId: null,
        bulkLotId: null,
        allocationIndex: null,
        allocationPence: 2500,
        status: "available",
        condition: "Near Mint", location: null, stickerNumber: "0101", privateNote: "", createdAt: "2026-07-18T12:00:00.000Z",
      },
      {
        id: "copy-preview-partial-2",
        printingId: printings[3].id,
        acquiredRecordId: "record-preview-partial-costs",
        soldRecordId: null,
        bulkLotId: null,
        ...
        allocationPence: 2500,
        status: "available",
        condition: "Near Mint", location: null, stickerNumber: "0102", privateNote: "", createdAt: "2026-07-18T12:00:01.000Z",
      },
      {
        id: "copy-preview-partial-3",
        printingId: printings[3].id,
        acquiredRecordId: "record-preview-partial-costs",
        soldRecordId: null,
        bulkLotId: null,
        ...
        allocationPence: null,
        status: "available",
        condition: "Lightly Played", location: null, stickerNumber: "0103", privateNote: "", createdAt: "2026-07-18T12:00:02.000Z",
      },
      {
        id: "copy-preview-ash",
...
export function deleteWishlistTarget(snapshot: RecordsSnapshot, targetId: string) {
  ...
  if (target.desiredQuantity === 0) {
    return {
      next: snapshot,
      result: { ok: false, message: "This card is not currently on your wishlist." } satisfies DataSourceResult,
    };
  }
  if (copies.length) {
    const availableCopyCount = copies.filter((copy) => copy.status === "available").length;
    const next = clone(snapshot);
    const nextTarget = next.targets.find((item) => item.id === targetId)!;
    nextTarget.desiredQuantity = 0;
    return {
      next,
      result: {
        ok: true,
...
export function updateCardSource(
  snapshot: RecordsSnapshot,
  update: CardSourceUpdate,
  metadata: ResolvedProductMetadata,
) {
  const target = snapshot.targets.find((item) => item.id === update.targetId);
  const printing = snapshot.printings.find((item) => (
    item.id === update.printingId && item.targetId === update.targetId
  ));
  if (!target || !printing) {
    return {
      next: snapshot,
      result: { ok: false, message: "This Card Printing is no longer in the current snapshot." } satisfies DataSourceResult,
    };
  }
  if (!metadata.rarity.trim()) {
    return {
      next: snapshot,
      result: { ok: false, message: "The new link did not return a rarity, so nothing was changed." } satisfies DataSourceResult,
    };
  }

  const next = clone(snapshot);
  const nextTarget = next.targets.find((item) => item.id === target.id)!;
  const nextPrinting = next.printings.find((item) => item.id === printing.id)!;
  const name = metadata.title.trim() || target.name;
  const rarity = metadata.rarity.trim();
  ...
  nextTarget.rarity = rarity;
  nextTarget.edition = edition;
  nextTarget.tcgplayerUrl = update.tcgplayerUrl.trim();
  nextTarget.imageUrl = imageUrl;
  nextPrinting.setName = setName;
  nextPrinting.setCode = setCode;
  nextPrinting.tcgplayerUrl = update.tcgplayerUrl.trim();
  nextPrinting.imageUrl = imageUrl;

  const copyIds = new Set(next.copies
    .filter((copy) => copy.printingId === printing.id)
    .map((copy) => copy.id));
  for (const record of next.records) {
    for (const line of record.lines) {
      if (!line.entityIds.some((entityId) => copyIds.has(entityId))) continue;
      line.name = name;
      line.detail = `${setCode || "Unknown code"} · ${edition} · ${rarity}`;
    }
  }
  next.attention = next.attention.filter((item) => (
    !(item.printingId === printing.id || (
      item.targetId === target.id && item.field !== "cost"
    ))
  ));
  return { next, result: { ok: true, id: target.id } satisfies DataSourceResult };
}
...
export function applyPurchase(snapshot: RecordsSnapshot, input: PurchaseInput) {
  ...
  if (input.kind === "card") {
    ...
    const allocationByCopyId = new Map([...ids].sort().map((copyId, index) => [copyId, allocations[index]!]));
    for (const copyId of ids) {
      const copy = next.copies.find((item) => item.id === copyId);
      if (copy) copy.allocationPence = allocationByCopyId.get(copyId)!;
    }
    lines.push(recordLine(
      ...
      card.quantity,
      ids,
      ordinaryPurchaseLineAllocation({ amountKnown: input.amountKnown !== false, amountPence: input.totalPence }),
      `${card.setCode || "Unknown code"} · ${card.edition} · ${card.rarity}`,
    ));
    ...
    for (const card of input.cards) {
      ...
      const values = indexes.map((index) => (
        input.amountKnown === false ? null : allocatePenceAt(input.totalPence, input.totalCardCount, index)
      ));
      ...
      lines.push(recordLine(
        ...
        card.quantity,
        ids,
        input.amountKnown === false ? null : values.reduce<number>((sum, value) => sum + (value ?? 0), 0),
  ...
  next.records.unshift({
    ...
    amountPence: input.amountKnown === false ? 0 : input.totalPence,
    amountKnown: input.amountKnown !== false,
    notes: input.notes,
    lines,
    revision: 1,
...
export function applyOpening(snapshot: RecordsSnapshot, input: OpeningInput) {
  ...
  if (!sealed) {
    ...
    next.records.unshift({
      ...
      source: input.source,
      listingUrl: null,
      amountPence: isGift || input.amountKnown === false ? 0 : input.totalPence,
      amountKnown: isGift || input.amountKnown !== false,
      notes: isGift ? "Gifted sealed product." : input.amountKnown === false ? "Cost unknown; recorded alongside a pack opening." : "Recorded alongside a pack opening.",
      lines: [recordLine("sealed", input.product.name, 1, [sealedId], isGift || input.amountKnown !== false ? (isGift ? 0 : input.totalPence) : null, isGift ? `Gift · £0 · ${input.product.edition}` : input.amountKnown === false ? `Cost unknown · ${input.product.edition}` : `£${(input.totalPence / 100).toFixed(2)} · ${input.product.edition}`)],
      revision: 1,
      createdAt: nowIso(),
    });
  }
...
export function updateRecordDetails(snapshot: RecordsSnapshot, recordId: string, update: RecordDetailsUpdate) {
  ...
  record.amountPence = nextAmountKnown ? update.amountPence : 0;
  record.amountKnown = nextAmountKnown;
  next.attention = record.amountKnown
    ? next.attention.filter((item) => item.id !== `attention-cost-${record.id}`)
    : next.attention;
  ...
  if (bulkLot) {
    for (const copy of next.copies.filter((item) => item.bulkLotId === bulkLot.id)) {
      if (copy.allocationIndex !== null) {
        copy.allocationPence = nextAmountKnown
          ? allocatePenceAt(update.amountPence, bulkLot.totalQuantity, copy.allocationIndex)
          : null;
      }
    }
    for (const line of record.lines.filter((item) => item.kind === "card")) {
      line.allocationPence = nextAmountKnown
        ? next.copies
    ...
    const cardLines = record.lines.filter((line) => line.kind === "card");
    if (cardLines.length === 1 && record.lines.length === 1) {
      const line = cardLines[0]!;
      const copies = next.copies
        .filter((copy) => line.entityIds.includes(copy.id))
        .sort((left, right) => left.id.localeCompare(right.id));
      if (copies.length !== line.quantity) {
        return { next: snapshot, result: { ok: false, message: "This Purchase's physical Copies no longer match its one card line." } satisfies DataSourceResult };
      }
      const allocations = ordinaryPurchaseCopyAllocations({
  ...
  } else if (record.type === "imported-acquisition" && record.lines.length === 1) {
    record.lines[0]!.allocationPence = ordinaryPurchaseLineAllocation({
      amountKnown: record.amountKnown !== false,
      amountPence: record.amountPence,
    });
...
export function replaceRecordCards(snapshot: RecordsSnapshot, recordId: string, cards: CardContentsInput[]) {
  ...
  if (record.type === "purchase" && !bulkLot) {
    ...
    const copies = line ? next.copies.filter((copy) => copy.acquiredRecordId === record.id) : [];
    if (
      record.lines.length !== 1
      ...
      || cards.length !== 1
      || !requested
      || requested.id !== line.id
    ) return { next: snapshot, result: { ok: false, message: "Ordinary card Purchases support one complete card line only." } satisfies DataSourceResult };
  }
  ...
  function addRecordCopies(card: CardContentsInput, quantity: number) {
    ...
    const values = indexes.map((index) => (
      editableRecord.amountKnown === false ? null : allocatePenceAt(editableRecord.amountPence, bulkLot.totalQuantity, index)
    ));
    return addCopies(
      next,
      { ...card, quantity },
  ...
  for (const card of cards) {
    ...
    if (!existingLine) {
      ...
      const allocation = record.amountKnown === false
        ? null
        : next.copies
    ...
    nextCardLines.push({
      ...
      allocationPence: bulkLot
        ? record.amountKnown === false ? null : next.copies
            .filter((copy) => entityIds.includes(copy.id))
            .reduce((sum, copy) => sum + (copy.allocationPence ?? 0), 0)
        : ordinaryPurchaseLineAllocation({ amountKnown: record.amountKnown !== false, amountPence: record.amountPence }),
  ...
  if (record.type === "purchase" && !bulkLot) {
    const [line] = nextCardLines;
    const copies = next.copies
      .filter((copy) => copy.acquiredRecordId === record.id)
      .sort((left, right) => left.id.localeCompare(right.id));
    const allocations = ordinaryPurchaseCopyAllocations({
      amountKnown: record.amountKnown !== false,
      amountPence: record.amountPence,
    ...
    for (const [index, copy] of copies.entries()) copy.allocationPence = allocations[index]!;
    if (line) line.allocationPence = ordinaryPurchaseLineAllocation({ amountKnown: record.amountKnown !== false, amountPence: record.amountPence });
  }
  if (bulkLot) {
...
export function removeCardCopy(snapshot: RecordsSnapshot, copyId: string) {
  ...
  if (line.quantity <= 1) {
    ...
    if (copy.bulkLotId) {
      line.allocationPence = record.amountKnown === false
        ? null
      ...
      for (const [index, item] of remainingCopies.sort((left, right) => left.id.localeCompare(right.id)).entries()) {
        item.allocationPence = allocations[index]!;
      }
    }
  }
  record.revision += 1;
  ...
  if (line.kind === "sealed") {
    ...
    for (const lot of lots) {
      ...
      for (const copy of lotCopies) {
        if (copy.allocationIndex !== null) {
          copy.allocationPence = record.amountKnown === false
            ? null
      ...
      for (const cardLine of record.lines.filter((item) => item.kind === "card")) {
        cardLine.allocationPence = record.amountKnown === false
          ? null
          : next.copies
