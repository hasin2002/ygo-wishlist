...
function SpendSummaryLink({
  ...
  return (
    ...
    >
      ...
      <span className="tabular-nums text-zinc-950">{formatCurrency(monthlyTotal)}</span>
    </Link>
  );
}
        ...
        {navigationVisible ? (
          <>
            <aside className={`fixed bottom-4 left-4 top-4 z-40 hidden flex-col rounded-xl border border-zinc-300 bg-[#fdfcf8] p-3 shadow-lg transition-[width] duration-200 lg:flex ${desktopMenuOpen ? "w-64" : "w-[72px]"}`}>
              ...
              <nav aria-label="Primary" className="mt-5 flex flex-1 flex-col gap-1.5" id={desktopNavId}>
                {visibleNavItems.map((item) => <PrimaryNavLink active={isActive(activePathname, item.href)} expanded={desktopMenuOpen} item={item} key={item.href} onSelect={selectNavigation} />)}
              </nav>
