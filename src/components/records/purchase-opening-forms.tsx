...
import { PurchaseCardSelection } from "@/components/records/purchase-card-selection";
...
export function PurchaseForm({ edit, onSaved }: { edit?: RecordFormEdit; onSaved: (recordId: string, warning?: string) => void }) {
  ...
  return (
    ...
    <form autoComplete="off" className="grid gap-4" onSubmit={(event) => event.preventDefault()}>
      ...
      <FormDraftStatus compact dirty={lifecycle.dirty} onDiscard={() => {
      ...
      <WizardProgress compact labels={wizardLabels} step={wizardStep} />

      {step === 1 ? <StepPanel step={step}><FormSection compact description="Choose one acquisition kind. Bulk can contain several identified cards, but the Purchase itself stays one lot." number={1} title="What did you buy?">
      ...
      {step === 2 ? <StepPanel step={step}><FormSection compact description={`Record the shared facts for this ${selectedKind?.label.toLowerCase() || "purchase"}.`} number={2} title="Purchase details">
      ...
      {step === 3 ? <StepPanel step={step}><FormSection compact description={draft.kind === "card" || draft.kind === "bulk" ? "Find your cards in the catalogue, then choose edition, condition and quantity." : "Check the product details and quantity."} number={3} title={`${selectedKind?.label || "Item"} details`}>
        {draft.kind === "card" ? <PurchaseCardSelection single rows={[draft.card]} onChange={(cards) => setDraft((current) => ({ ...current, card: cards[0] ?? blankCardContents() }))} /> : null}
      ...
      {step === 4 ? <StepPanel step={step}><div className="grid gap-4"><PreviewNotice label={source.mode === "preview" ? "Preview only." : "Review before saving."}>This is a read-only review. Nothing has been saved; only the confirmation button below {edit ? "updates this existing Purchase" : `creates the ${source.mode === "preview" ? "preview " : ""}purchase`}.</PreviewNotice><FormSection compact description="Check the purchase facts and item details. Use Edit to correct a section before confirming." number={4} title="Review purchase">
      ...
      <WizardActions compact finalLabel={edit ? "Update purchase" : `Confirm${source.mode === "preview" ? " preview" : ""} purchase`} onBack={() => { setError(null); setStep((current) => Math.max(firstVisibleStep, current - 1)); }} onConfirm={submit} onNext={nextStep} pending={pending || (step === 4 && cardPricing.pendingCount > 0)} pendingLabel={cardPricing.pendingCount > 0 && !pending ? "Finishing estimates…" : "Saving…"} step={wizardStep} totalSteps={wizardTotalSteps} />
    </form>
    </DraftHydrationBoundary>
  );
}
...
export function OpeningForm({ edit, onSaved }: { edit?: RecordFormEdit; onSaved: (recordId: string, warning?: string) => void }) {
  ...
  return (
    ...
    <form autoComplete="off" className="grid gap-4" onSubmit={(event) => event.preventDefault()}>
      ...
      {step === 1 ? <StepPanel step={step}><FormSection compact description="Choose the kind of opening before adding any product details." number={1} title="How are you recording this opening?">
      ...
      {step === 2 ? <StepPanel step={step}><FormSection compact description={draft.acquisitionMode === "tracked" ? "Find the exact sealed unit you are opening." : "Add the sealed product and the shared details for this opening."} number={2} title="Opening details">
      ...
      {step === 3 ? <StepPanel step={step}><FormSection compact description="Choose each pulled card from the catalogue. Its physical Copies stay tied to this opening." number={3} title="Pulled cards"><PurchaseCardSelection onChange={(pulls) => setDraft((current) => ({ ...current, pulls }))} rows={draft.pulls} /></FormSection></StepPanel> : null}

      {step === 4 ? <StepPanel step={step}><div className="grid gap-4"><PreviewNotice label={source.mode === "preview" ? "Preview only." : "Review before saving."}>This is a read-only review. Nothing has been saved; only the confirmation button below {edit ? "updates this existing Pack Opening" : `creates the ${source.mode === "preview" ? "preview " : ""}opening`}.</PreviewNotice><FormSection compact description="Check the product, source, date, pulled cards, and notes. Use Edit to correct a section." number={4} title="Review opening">
      ...
      <WizardActions compact finalLabel={edit ? "Update opening" : `Confirm${source.mode === "preview" ? " preview" : ""} opening`} onBack={() => { setError(null); setStep((current) => Math.max(firstVisibleStep, current - 1)); }} onConfirm={submit} onNext={nextStep} pending={pending || (step === 4 && cardPricing.pendingCount > 0)} pendingLabel={cardPricing.pendingCount > 0 && !pending ? "Finishing estimates…" : "Saving…"} step={wizardStep} totalSteps={wizardTotalSteps} />
    </form>
    </DraftHydrationBoundary>
  );
}
