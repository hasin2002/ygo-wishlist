...
export function OpeningForm({ edit, onSaved }: { edit?: RecordFormEdit; onSaved: (recordId: string, warning?: string) => void }) {
  ...
  return (
    ...
    <form autoComplete="off" className="grid gap-4" onSubmit={(event) => event.preventDefault()}>
      ...
      {step === 3 ? <StepPanel step={step}><FormSection description="Choose each pulled card from the catalogue. Its physical Copies stay tied to this opening." number={3} title="Pulled cards"><PurchaseCardSelection onChange={(pulls) => setDraft((current) => ({ ...current, pulls }))} rows={draft.pulls} /></FormSection></StepPanel> : null}
