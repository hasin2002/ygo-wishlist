...
test("Inventory list state parses valid filters and safely normalizes invalid values", () => {
  assert.deepEqual(parseInventoryListState(new URLSearchParams("kind=cards&card=Blue-Eyes&copies=multiple&status=owned&rarity=Ultra+Rare&rarity=Secret+Rare&edition=1st+Edition&page=3")), {
    kind: "cards",
    card: "Blue-Eyes",
...
test("Inventory URLs preserve list state and add an optional selected physical Copy", () => {
  const state = parseInventoryListState(new URLSearchParams("kind=cards&card=Ash+Blossom&copies=multiple&status=wishlist&rarity=Super+Rare&rarity=Ultra+Rare&edition=1st+Edition&page=2"));
